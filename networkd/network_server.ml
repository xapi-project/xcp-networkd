...
module Sriov = struct
	...
	let config_sriov ?(disable=false) dev  =
		let open Rresult.R.Infix in
		Sysfs.get_driver_name_err dev >>= fun driver ->
		match Modprobe.get_vf_param driver with
		| Some vf_param ->
			debug "enable SR-IOV on a device: %s via modprobe" dev;
			begin
				if disable then Rresult.Ok 0 else Modprobe.get_maxvfs driver
			end >>= fun maxvfs ->
			Modprobe.config_sriov driver vf_param maxvfs >>= fun _ ->
			Ok Modprobe_successful_requires_reboot
		| None ->
			debug "enable SR-IOV on a device: %s via sysfs" dev;
			let maxvfs = if disable then 0 else Sysfs.get_sriov_maxvfs dev in
			Sysfs.set_sriov_numvfs dev maxvfs >>= fun _ ->
			Ok Sysfs_successful

	let enable _ dbg ~name =
		Debug.with_thread_associated dbg (fun () ->
			debug "Enable network SR-IOV by name: %s" name;
			match config_sriov name with
	...
	let disable _ dbg ~name =
		Debug.with_thread_associated dbg (fun () ->	
			...
			match config_sriov ~disable:true name with
