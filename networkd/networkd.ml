...
let options = [
	...
	"json-rpc-read-timeout", Arg.Int (fun x -> Jsonrpc_client.json_rpc_read_timeout := Int64.(mul 1000L (of_int x))), (fun () -> Int64.(to_string (div !Jsonrpc_client.json_rpc_read_timeout 1000L))), "JSON RPC response read timeout value in ms";
	"json-rpc-write-timeout", Arg.Int (fun x -> Jsonrpc_client.json_rpc_write_timeout := Int64.(mul 1000L (of_int x))), (fun () -> Int64.(to_string (div !Jsonrpc_client.json_rpc_write_timeout 1000L))), "JSON RPC write timeout value in ms";
]
